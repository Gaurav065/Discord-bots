...
class InvalidReapeatMode(commands.CommandError):
    pass
...
class Music(commands.Cog, wavelink.WavelinkMixin):
    ...
    async def play_com(self, ctx, *, query: t.Optional[str]):
        ...
        if query is None:
            ...
            await ctx.send("Music/Audio resumed")
    ...
    @play_com.error
    async def play_com_error(self, ctx, exc):
        if isinstance(exc,QueueIsEmpty):
            await ctx.send("No songs found in the queue, please add audio/songs to play")
        elif isinstance(exc, NoVoiceChannel):
            await ctx.send("No voice channel was provided")
        
    @commands.command(name="pause")
    async def pause_com(self,ctx):
        player = self.get_player(ctx)
        ...
        await ctx.send("Music/Audio paused")
    
    @pause_com.error
    async def pause_com_error(self,ctx, exc):
        if isinstance(exc, PlayerIsAlreadyPaused):
            await ctx.send("The player is already paused")

    @commands.command(name="stop")
    async def stop_com(self, ctx):
        player = self.get_player(ctx)
        player.queue.clr()
        await player.stop()
    ...
    async def next_com(self, ctx):
        player = self.get_player(ctx)
        if not player.queue.upcoming_tracks:
            raise NoMoreTracks
        
        await player.stop()
        await ctx.send("Playing next Music/audio in the list.")

    @next_com.error
    async def next_com_error(self, ctx, exc):
        if isinstance(exc, QueueIsEmpty):
            await ctx.send("It was not possible to skip as the queue was empty")
        
        elif isinstance(exc, NoMoreTracks):
            await ctx.send("There are no More tracks to play.")

    @commands.command(name="previous", aliases=["last"])
    async def previous_com(self, ctx):
        player = self.get_player(ctx)
        if not player.queue.history:
            raise NoPreviousTracks
        player.queue.position -= 2
        await player.stop()
        await ctx.send("Playing previous/last Music/audio in the list.")

    @previous_com.error
    async def previous_com_error(self, ctx, exc):
        if isinstance(exc, QueueIsEmpty):
            await ctx.send("It was not possible to play the last/previous track as the queue was empty")
    ...
    async def repeat_com(self, ctx, mode: str):
        if mode not in ("none", "1", "all"):
            raise InvalidReapeatMode
        
        player = self.get_player(ctx)
        player.queue.set_repeat_mode(mode)
        await ctx.send(f"The requested loop/reapeat mode has been set to {mode}")


    @commands.command(name="queue", aliases=["q"])
    async def queue_com(self, ctx, show: t.Optional[int] = 10):
        player = self.get_player(ctx)
        ...
        embed = discord.Embed(
            title="Queue",
            description=f"Displaying next {show} tracks",
            colour =ctx.author.colour,
        ...
        embed.add_field(name="Currently Playing", 
                value=getattr(player.queue.current_track,
                "title",
                "No tracks Currenlty Playing."),
                inline=False)
        if upcoming_tracks := player.queue.upcoming_tracks:
            embed.add_field(
                name="Next",
                value="\n".join(t.title for t in upcoming_tracks[:show]),
                inline=False
            )
        msg = await ctx.send(embed=embed)
    ...
    async def queue_command_error(self, ctx , exc):
        if isinstance(exc, QueueIsEmpty):
            await ctx.send("The queue is currently empty, please play an audio to see the queue")
    ...
    @volume_group.command(name="down", aliases=["-"])
